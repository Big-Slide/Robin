...
class Manager(Base):
    ...
    input1_path = Column(String(4000), nullable=True)
    input2_path = Column(String(4000), nullable=True)
    input_params = Column(String(20000), nullable=True)
    priority = Column(Integer, nullable=True)
    model = Column(String(255), nullable=True)
    status = Column(Enum(WebhookStatus), nullable=False, default=WebhookStatus.pending)
    result = Column(String(20000), nullable=True)
    result_path = Column(String(4000), nullable=True)

...
class Manager(Base):
    """
    Database model for managing facial expression analysis requests
    """
    __tablename__ = "facetotxt_manager"

    id = Column(String(255), primary_key=True, default=generate_uuid)
    request_id = Column(String(255), nullable=False, unique=True)

    # Fields specific to facial expression analysis
    ...
    result = Column(JSON, nullable=True)  # Store facial expression results as JSON

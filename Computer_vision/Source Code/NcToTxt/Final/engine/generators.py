...
if os.environ.get("MODE", "dev") == "prod":
    ...
    models_dir = "../../../../Models/NcToTxt"
id_detect_model = f'{models_dir}/detect_id_card.pt'

class IDCardProcessor:
    def __init__(self):
        # Initialize YOLO model
        self.id_card_model = YOLO(id_detect_model)
    ...
    def detect_and_process_id_card(self, image_input):
        """
        Process an image (file path or numpy array), detect ID card, perform OCR,
        and return annotated image with text boxes.
        Includes skew correction for better OCR accuracy.
        """
        # Load image
        ...
        # Detect ID card
        id_card_results = self.id_card_model(image)
        ...
        for result in id_card_results:
            for box in result.boxes:
                # Get bounding box coordinates and convert to regular Python ints
                x1, y1, x2, y2 = map(int, box.xyxy[0].tolist())

                # Draw rectangle around ID card
                ...
                if lines is not None:
                    ...
                    if angles:
                        ...
                        center = tuple(np.array(card_image.shape[1::-1]) / 2)
                        rotation_matrix = cv2.getRotationMatrix2D(center, median_angle, 1.0)
                        card_image = cv2.warpAffine(
                            card_image,
                            rotation_matrix,
                            card_image.shape[1::-1],
                            flags=cv2.INTER_CUBIC,
                            borderMode=cv2.BORDER_REPLICATE
                        )

                # Resize corrected image to target size
                card_image = self.resize_maintain_aspect(card_image, self.target_size)

                # Perform OCR on the resized image
                ocr_results = self.reader.readtext(card_image)

                # Draw OCR results on the resized image
                annotated_card = self.draw_ocr_results(card_image, ocr_results)
                ...
                text_data.append({
                    'bbox': [int(x1), int(y1), int(x2), int(y2)],  # Convert to Python ints
                    'texts': text_entries,
                    'skew_corrected': skew_corrected
                })

                processed_images.append(annotated_card)

        return image, processed_images, text_data
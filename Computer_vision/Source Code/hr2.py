...
posture_scores = []  # Track posture (upright vs. slouching)
eye_contact_frames = 0  # Track eye contact
...
while cap.isOpened():
    ...
    if face_results.multi_face_landmarks:
        for face_landmarks in face_results.multi_face_landmarks:
            ...
            if face_roi.size != 0:  # Ensure the ROI is not empty
                ...
                print("Raw Predictions:", emotion_prediction)  # Debugging output
                emotion_label = emotion_labels[np.argmax(emotion_prediction)]
                print("Predicted Emotion:", emotion_label)  # Debugging output
    ...
    if pose_results.pose_landmarks:
        ...
        # Detect posture (upright vs. slouching)
        ...
        # Calculate shoulder-hip angle (simple heuristic for posture)
        shoulder_hip_angle = abs(left_shoulder.y - left_hip.y) + abs(right_shoulder.y - right_hip.y)
        posture_scores.append(shoulder_hip_angle)
...
posture_score = np.mean(posture_scores)  # Average posture score
print(f"Speaking Percentage: {speaking_percentage:.2f}%")
print(f"Gesture Count: {gesture_count}")
print(f"Posture Score: {posture_score:.2f} (lower is better)")
